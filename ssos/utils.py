...
def init_argparse():
    ...
    parser.add_argument('fields', nargs='+',
                        help=f'Path to directory of field to be searched')
    parser.add_argument('-c', '--config', action='store', dest='set_up',
                        help=f'Path to configuration file', nargs=1)
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
        ...
        for line in default:
            ...
            if val in ['True', 'False']\
               and param not in ['WEIGHT_IMAGES']:
                group.add_argument(f'--{param}', metavar='bool',
                                   action='store',
                                   help=f'Override {param} setting. Must be '
                                        f'True or False.', choices=['True',
                                                                    'False'])
            # If parameter argument
            else:
                group.add_argument(f'--{param}', metavar='value',
                                   action='store',
    ...
    for param in ['ASTREF_CATALOG', 'CROSSID_RADIUS']:
        group.add_argument(f'--{param}', metavar='value',
                           action='store', help=f'Override {param} setting.')
...
def init_logger(args, log_dir):
    ...
    log_file = 'ssos_{:s}.log'.format(time.strftime('%Y%m%d%H%M%S',
                                                    start_time))
...
def check_wcs(images, sci_ext, log):
    ...
    for i, image in enumerate(images):
        ...
        kws = ['CTYPE1', 'CTYPE2', 'CRVAL1', 'CRVAL2',
...
def compute_image_center(header):
    ...
    patterns = [r'PV[0-9]+_[0-9]+', r'TR[0-9]+_[0-9]+',
                r'WAT[0-9]+_[0-9]+', r'LTM[0-9]+_[0-9]+']
...
def create_clean_image(image, tmp_path, update=False):
    ...
    with fits.open(image, 'update' if update else 'readonly',
                   output_verify=False) as exp:
        for hdu in exp:
...
def convert_to_mpc(infile):
    ...
    if 'MAG_CALIB' not in data.columns:
        mag_dec = input(f'No calibrated magnitude found in MAG_CALIB column. '
                        f'Leave blank? [Y|n]:')
    ...
    with open(outfile, 'w') as output:
        ...
        for index, row in tqdm(data.iterrows(), total=data.shape[0],
                               unit='obs', desc='Converting to MPC format'):
            ...
            if len(line) != 81:  # 80 + newline character
                print(f'Error: This line has {len(line) - 1} instead of '
                      f'80 characters:')
                print(line)
                sys.exit()
            output.write(line)
